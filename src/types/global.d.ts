...
type AppSection = "home" | "grammar" | "vocabulary" | "daily" | "travel" | "games" | "exercises" | "profile";
type LessonStatus = "not_started" | "in_progress" | "completed";
type PathId = "general" | "travel" | "daily" | string;
...
type UiTexts = Record<Language, UiTextSet>;
...
interface ChoiceExerciseQuestion extends BaseExerciseQuestion {
    type: "mcq" | "binary";
    options: string[];
    correct: number;
}

interface FillBlankExerciseQuestion extends BaseExerciseQuestion {
    type: "fill_blank";
    correct: string;
}

interface OrderingExerciseQuestion extends BaseExerciseQuestion {
    type: "ordering";
    words: string[];
    correct: string[];
}

type ExerciseQuestion = ChoiceExerciseQuestion | FillBlankExerciseQuestion | OrderingExerciseQuestion;
type ExerciseAnswer = number | string | string[];
...
type LessonSection = LessonContentSection | ExerciseSection;
...
interface TravelVocabSection extends TravelBaseSection {
    type: "vocab";
    words: TravelVocabWord[];
}

interface TravelTipsSection extends TravelBaseSection {
    type: "tips";
    tips: TravelTip[];
}

interface TravelLessonContentSection extends TravelBaseSection {
    type: "lesson";
    content?: string;
    examples?: LessonExample[];
    table?: LessonTable;
...
interface TravelExerciseSection extends TravelBaseSection {
    type: "exercise";
    questions: ExerciseQuestion[];
    displayCount?: number;
}

type TravelSection = TravelVocabSection | TravelTipsSection | TravelLessonContentSection | TravelExerciseSection;
type ExerciseSectionInput = ExerciseSection | TravelExerciseSection;
...
interface VocabStories {
    ...
    literary?: VocabStory;
    hard?: VocabStory;
    [key: string]: VocabStory | undefined;
}
...
interface NewsArticle extends NewsIndexItem {
    imageAlt?: string;
    week?: number;
    year?: number;
    content: {
...
interface SearchGrammarItem extends GrammarLessonIndex {
    content?: string;
    example?: string;
}

interface SearchVocabWord extends VocabWord {
    level: Level;
    packId: string;
}
