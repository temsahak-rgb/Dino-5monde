// ===============================
// ✈️ موتور درس‌های سفر (نسخه نهایی)
// ===============================

let travelCache = {};

// بارگذاری فهرست درس‌ها
...
async function showTravelPage() {
    ...
    if (lessons.length === 0) {
        ...
        lessons.forEach(lesson => {
            html += `<div style="background:#fff;border:1px solid #e0e0e0;border-radius:8px;padding:20px;cursor:pointer;transition:all 0.2s;" onclick="showTravelLesson('${lesson.id}')" onmouseover="this.style.borderColor='#087F5B'" onmouseout="this.style.borderColor='#e0e0e0'">`;
...
async function showTravelLesson(lessonId) {
    ...
    if (!lesson) {
        app.innerHTML = renderNavbar() + `<div style="max-width:600px;margin:0 auto;padding:60px 20px;text-align:center;">
            ...
            <button onclick="showTravelPage()" style="padding:12px 24px;background:#087F5B;color:#fff;border:none;border-radius:6px;cursor:pointer;font-weight:700;">${lang === "fa" ? "بازگشت" : "Retour"}</button>
        </div>`;
        return;
    }
    ...
    if (!lesson.miniLessons || lesson.miniLessons.length === 0) {
        app.innerHTML = renderNavbar() + `<div style="max-width:600px;margin:0 auto;padding:60px 20px;text-align:center;">
            ...
            <p style="font-size:16px;color:#777;margin-bottom:20px;">${lang === "fa" ? "محتوای این درس به زودی اضافه می‌شود" : "Le contenu sera bientôt ajouté"}</p>
            <button onclick="showTravelPage()" style="padding:12px 24px;background:#087F5B;color:#fff;border:none;border-radius:6px;cursor:pointer;font-weight:700;">${lang === "fa" ? "بازگشت" : "Retour"}</button>
        </div>`;
        return;
    }
    ...
    lesson.miniLessons.forEach((mini, idx) => {
        ...
        html += `<button onclick="showMiniLesson('${lessonId}', ${idx})" style="padding:12px;background:#fff;border:1px solid #e0e0e0;border-radius:6px;cursor:pointer;text-align:right;transition:all 0.2s;" onmouseover="this.style.borderColor='#087F5B'" onmouseout="this.style.borderColor='#e0e0e0'">`;
