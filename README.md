...
- 🔁 révisions actionnables sur `/practice/review` à partir des erreurs, mots faibles et derniers scores sous 80 % ;
...
L’atelier éditorial est disponible sur `/admin/content`. Il permet de rechercher les contenus du serveur, consulter leurs révisions, créer ou modifier un contenu, importer l’export Grammaire en brouillons et publier une révision choisie. Les Actualités disposent d’un éditeur guidé : titres et chapôs bilingues, niveau, date, illustration accessible, textes complet et simplifié, sources, vocabulaire et connexions vers les leçons de grammaire. Un tableau à voyants et un aperçu réel FR/FA rendent les erreurs visibles avant la sauvegarde ; le JSON canonique reste accessible dans un panneau avancé.

Enregistrer et publier sont deux actions séparées : aucune sauvegarde ne devient publique implicitement. Une actualité incomplète peut rester en brouillon, mais sa publication est refusée à la fois dans l’interface et par PostgreSQL. Le serveur vérifie notamment l’identifiant partageable, la date, la plage CECRL, les sources et l’existence publiée des leçons ou packs explicitement liés. Le tableau de bord affiche les contenus publiés, les brouillons en attente et la couverture de publication ; il reste utilisable sur mobile.
